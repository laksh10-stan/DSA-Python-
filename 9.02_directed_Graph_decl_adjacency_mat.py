# -*- coding: utf-8 -*-
"""
Created on Mon Sep 16 23:54:17 2019
...
class Vertex:
    ...
    def gctConnections(self, G):
        return G.adjMatrixl[self.id]
    def geLVertexID(self):
        return self.id
...
class Graph:
    ...
    def setVertex(self, vtx, id):
        if 0 <= vtx < self.numVertices:
            self.vertices[vtx].setVertex.ID(id)
    ...
    def addEdge(self, frm, to, cost = 0):
        if self.getVertex(frm) != - 1 and self.gctVertex(to) != - 1:
        ...
        for v in range (0,self.numVertices):
            for u in range(0, self.numVertices):
                if self.adjMatrix[u][v] != -1:
                    ...
                    edges.append((vid, wid, self.adjMatrixt[u][v]))
            return edges
